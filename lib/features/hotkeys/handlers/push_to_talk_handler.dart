...
class PushToTalkHandler {
  ...
  static void handleKeyDown() async {
    ...
    final pushToTalkEnabled = Hive.box('settings').get('push_to_talk_enabled', defaultValue: true) as bool;
    ...
    if (RecordingOverlayPlatform.isRecordingInProgress && !_isPushToTalkRecordingActive) {
  ...
  static void handleKeyUp() async {
    ...
    try {
      ...
      if (_pushToTalkRecordedFilePath != null) {
        final apiKey = Hive.box('settings').get('groq_api_key');
        await _transcribeAndCopyToClipboard(_pushToTalkRecordedFilePath!, apiKey);
      }
    } catch (e) {
      if (kDebugMode) {
